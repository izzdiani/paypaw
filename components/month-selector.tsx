...
export function MonthSelector({
  ...
  return (
    <section className="rounded-2xl bg-white/85 p-2.5 shadow-soft">
      <div className="mb-2 grid grid-cols-[auto_1fr_auto] items-center gap-2">
        <button
          aria-label="Previous year"
          onClick={() => moveYear(-1)}
          className="flex h-8 w-8 items-center justify-center rounded-full bg-paw-blush text-base font-bold text-paw-purple transition hover:bg-paw-lavender"
          type="button"
        >
          &lt;
        </button>

        <div className="justify-self-center rounded-full bg-paw-purple px-5 py-1 text-center text-sm font-bold text-white">
          {activeYear}
        </div>
        ...
        {months.map((month, index) => {
          ...
          return (
            <button
              ...
              className={`relative flex h-8 items-center justify-center rounded-lg text-xs font-bold transition ${
                isActive
            ...
            >
              {month}
              {hasBudget && !isActive ? (
                <span className="absolute bottom-0.5 h-1 w-1 rounded-full bg-paw-purple" />
              ) : null}
            </button>
          );
        })}
      ...
      <button
        ...
        className="mt-2 w-full rounded-lg bg-red-100 px-4 py-1.5 text-xs font-bold text-red-700 transition hover:bg-red-200"
        type="button"
      >
        Delete Month
      </button>
